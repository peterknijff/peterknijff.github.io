...
>
  <channel>
    <atom:link href="http://wwww.peterknijff.nl/rss.xml" rel="self" type="application/rss+xml" />
    <title>${title}</title>
    ...
    ${posts
        ...
        <item>
          ...
          ${
						post.coverImage
							? `<media:thumbnail xmlns:media="http://search.yahoo.com/mrss/" url="${siteBaseUrl}/${post.coverImage.png}"/>`
							: ''
					}
          ${
						post.coverImage
							? `<media:content xmlns:media="http://search.yahoo.com/mrss/" medium="image" url="${siteBaseUrl}/${post.coverImage.png}"/>`
							: ''
					}          
        </item>
      `
